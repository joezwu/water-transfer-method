...
parser.add_argument('--VirtualSite',  required=False,  type=str, default=None,
        help='Adds Three Particle Average Site: "index1, index2, index3, weight1, weight2, weight3"')
...
#adds virtual site
virtualsite = args['VirtualSite']
vs_indexes = [int(virtualsite.split()[0]), int(virtualsite.split()[1]), int(virtualsite.split()[2])]
vs_weights = [float(virtualsite.split()[3]), float(virtualsite.split()[4]), float(virtualsite.split()[5])]
...
print('Virtual Site indexes:               ', virtualsite)
...
############################################
#                                          #
#   READ AND CHARACTERIZE VIRTUAL SITE     #
#                                          #
############################################

d = np.linalg.norm(np.asarray(rcpt_positions[2655]/unit.nanometer)-np.asarray(rcpt_positions[vs_indexes[1]]/unit.nanometer))
r0 = np.sqrt(np.sum(np.square(np.asarray(rcpt_positions[vs_indexes[0]]/unit.nanometer))))
r1 = np.sqrt(np.sum(np.square(np.asarray(rcpt_positions[vs_indexes[1]]/unit.nanometer))))
r2 = np.sqrt(np.sum(np.square(np.asarray(rcpt_positions[vs_indexes[2]]/unit.nanometer))))
h = d / np.sqrt(
        4*np.square(r1) - 
        4*r1*(r0+r2) +
        np.square(r0 + r2)
